<?php 
    ...
    if (isset($_REQUEST['update_user'])) {
        try {
            ...
            $select_stmt = $db->prepare("SELECT * FROM addmoney WHERE username = :uname  , money = :money ");
            $select_stmt->bindParam(':uname', $username);
            $select_stmt->bindParam(':money', $money);
            $select_stmt->execute();
    ...
    if (isset($_REQUEST['btn_update'])) {
        ...
        $money_up = $_REQUEST['txt_money'];
        

        if (empty($username_up)) {
            $errorMsg = "Please Enter username";
        } else if (empty($money_up)) {
            $errorMsg = "Please Enter money";   
        } else {
            try {
                if (!isset($errorMsg)) {
                    $update_stmt = $db->prepare("UPDATE addmoney SET username = :uname_up, money = :money_up");
                    $update_stmt->bindParam(':uname_up', $username_up);
                    $update_stmt->bindParam(':money_up', $money_up);
    ...
    <form method="post" class="form-horizontal mt-5">
            ...
            <div class="form-group text-center">
                <div class="row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="txt_money" class="form-control" value="<?php echo $money; ?>">
                    </div>
                </div>
            </div>
            <div class="form-group text-center">
